...
#include "../hopper/flash_bwd_preprocess_kernel_f32.h"
#include "../hopper/flash_bwd_postprocess_kernel.h"
#include "../hopper/mainloop_bwd_sm80_f32.hpp"   // TYPE PROVIDER ONLY
#include "../hopper/epilogue_bwd.hpp"
#include "flex_flash_attention_bwd_kernel_sm80_f32.h"
...
void run_mha_flex_flash_bwd_hdim32(FlexFlashAttentionBwdParams &params, hggcStream_t stream) {
    // ISOLATED fp32 narrow-headdim bucket.  The hdim96 fp32 config's atom
    // layout (2,4,4) is validated here (grads bitwise-equal to the 64-bucket
    // run at TF32 precision).  NOTE: the hdim64 config (4,4,4) compiles at
    // width 32 but produces garbage grads — its M/N tiling carries a hidden
    // width-64 assumption; width 16 fails the MmadKVEvenN asserts outright.
...
void run_mha_flex_flash_bwd_hdim96(FlexFlashAttentionBwdParams &params, hggcStream_t stream) {
    if constexpr (sizeof(T) == 4) {
        // ISOLATED fp32 kernel set: 4-byte elements double the smem footprint,
        // so Stages_dO drops to 1 (dO shares Q's stage pipeline cadence via
        // Q_dO_same_stages=false). 16-bit keeps the validated config below.
        run_mha_flex_flash_bwd_tile<Arch, T, 96, 64, 128, 1, 2, false, false, false, 2, 2, 4, 4, false, Is_dropout>(params, stream);
    } else {
...
void run_mha_flex_flash_bwd_hdim128(FlexFlashAttentionBwdParams &params, hggcStream_t stream) {
    ...
    if constexpr (sizeof(T) == 4) {
        // ISOLATED fp32 kernel set: the 16-bit kBlockM=48/kBlockN=128 tile is
        // CVT/AIU-geometry and smem-optimized for 2-byte elements; with float
        // it exceeds the 256KB dynamic smem cap (launch fails with invalid
        // argument). P4 sweep (causal s1024/s4096/GQA-4:1, 2026-08): the
        // 64x96 Stages_dO=1/Stages=2 tile beat 64x64 by ~12% at s4096 and
        // keeps the 16-bit config untouched (both fit 1 CTA/CU).
        run_mha_flex_flash_bwd_tile<Arch, T, 128, 64, 96, 1, 2, false, false, false, 2, 4, 2, 2, false, Is_dropout>(params, stream);
    } else {
...
void run_mha_flex_flash_bwd_hdim192(FlexFlashAttentionBwdParams &params, hggcStream_t stream) {
    if constexpr (sizeof(T) == 4) {
        // ISOLATED fp32 kernel set: Stages=Stages_dO=1 + no dKV_swapAB keeps
        // the 4-byte buffers under the 256KB dynamic smem cap (~230KB); the
        // 16-bit swap is a CVT artifact. 16-bit keeps the validated config.
        run_mha_flex_flash_bwd_tile<Arch, T, 192, 64, 64, 1, 1, false, false, false, 2, 4, 2, 2, false, Is_dropout>(params, stream);
    } else {
...
void run_mha_flex_flash_bwd_hdim256(FlexFlashAttentionBwdParams &params, hggcStream_t stream) {
    if constexpr (sizeof(T) == 4) {
        // ISOLATED fp32 kernel set: kBlockN=32 halves the K/V/PdS footprint
        // so the 4-byte buffers fit the 256KB dynamic smem cap (~210KB).
        run_mha_flex_flash_bwd_tile<Arch, T, 256, 64, 32, 1, 1, false, false, false, 2, 4, 2, 2, false, Is_dropout>(params, stream);
    } else {
        run_mha_flex_flash_bwd_tile<Arch, T, 256, 64, 32, 1, 1, false, false, false, 2, 4, 2, 2, false, Is_dropout>(params, stream);
    }
}
...
void run_mha_flex_flash_bwd_(FlexFlashAttentionBwdParams &params, hggcStream_t stream) {
    ...
    BOOL_SWITCH(params.p_dropout_in_uint8_t < 255, Is_dropout, [&] {
        // No hdim16 bwd bucket: at width 16 the per-thread MMA fragment
        // (kBlockN*kHeadDim/NumMmaThreads = 8) can never match the 4-wide
        // accum copy partitioning — probed 2026-08 with swapAB/kBlockN
        // variants, all hit the same 2:1 static_assert.  hdim32 is the
        // narrowest expressible width.
