...
// CVT_LAYOUT comes from utils.h so that this file and mainloop_bwd_sm80.hpp can never disagree.
...
struct CollectiveEpilogueBwd {
...
#else
    using SmemLayoutAtomdKV = SmemLayoutAtomdKVSTG;
#endif
    ...
    struct CUTE_ALIGNAS(SmemAlignmentdKV) TensorStorage {
    ...
    store(Params const& params,
        ...
        // if (cute::thread0()) {
        //     printf("==== epilogue dV layout (thread%d) ====\n", thread_idx);
        //     printf("taccdVrdV: "); print(taccdVrdV.layout()); printf("\n");
        //     printf("taccdVsdV: "); print(taccdVsdV.layout()); printf("\n");
        //     printf("smem_tiled_copy_dKV: "); print(smem_tiled_copy_dKV); printf("\n");
        //     printf("sdV: "); print(sdV.layout()); printf("\n");
        //     printf("========================================\n");
        // }
        // if (cute::thread0()) {
        //     printf("==== taccdVrdV values (thread%d, size=%d) ====\n", thread_idx, (int)size(taccdVrdV));
        //     printf("taccdVrdV layout: "); print(taccdVrdV.layout()); printf("\n");
        //     for (int m = 0; m < size<1>(taccdVrdV); m++) {
        //         for (int n = 0; n < size<2>(taccdVrdV); n++) {
        //             printf("  [m=%d,n=%d]: ", m, n);
        //             for (int v = 0; v < size<0>(taccdVrdV); v++) {
        //                 printf("%.4f ", (float)taccdVrdV(v, m, n));
        //             }
        //             printf("\n");
        //         }
        //     }
        ...
        // if (cute::thread0()) {
        //     static constexpr int kBlockN = get<1>(TileShape_MNK{});
        //     const Element* sdV_ptr = reinterpret_cast<const Element*>(cute::raw_pointer_cast(sdV.data()));
        //     int total = kBlockN * kHeadDim;
        //     printf("==== sdV after copy (total=%d, 64 per line) ====\n", total);
        //     for (int idx = 0; idx < total; idx++) {
        //         printf("%.4f ", (float)sdV_ptr[idx]);
        //         if ((idx + 1) % 64 == 0) printf("\n");
        //     }
        //     printf("================================================\n");
        // }
        if constexpr (Use_TMA) {
...
struct CollectiveEpilogueBwdGQA {
    ...
    struct CUTE_ALIGNAS(SmemAlignment) TensorStorageTMA {
    ...
    store(Params const& params,
        ...
        if constexpr (Use_TMA) {
            ...
            // (Undo_CVT_HeaddimSwap).  Redirecting the atomicAdd to another thread's slice does not
