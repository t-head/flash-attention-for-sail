...
// Undo_CVT_HeaddimSwap: the mainloop CVT + swizzled smem load path leaves the dK/dV accumulators
...
template <class TileShape_MK_, class Element, class ElementAccum, class ArchTag_, int kNThreads, class TiledMma, bool dQ_swapAB,
          bool Undo_CVT_HeaddimSwap = false>
...
public:
    ...
    static_assert(!Undo_CVT_HeaddimSwap || (kGmemElemsPerLoad == 8 && kGmemThreadsPerRow == 32),
                  "Undo_CVT_HeaddimSwap assumes 8 elements per thread and 32 threads per row");
    ...
    operator()(Params const& params, char* smem_buf) {
        ...
        auto gmem_thr_copy_dQ_src = [&] {
            if constexpr (Undo_CVT_HeaddimSwap) {
