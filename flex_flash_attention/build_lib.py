...
THIS_DIR = Path(__file__).resolve().parent          # flex_flash_attention/
REPO_DIR = THIS_DIR.parent                          # fa repo root
HOPPER_DIR = REPO_DIR / "hopper"                    # hopper/ (pristine upstream baseline)
...
def sources():
    ...
    # needed by SDPA) and plus the SDPA glue TU.  The sources.py manifest is
    # rooted at hopper/ for the wheel build; the flex_flash_attention/ prefix
    # resolves from the repo root instead (it is a sibling of hopper/).
    srcs = [s for s in flex_flash_sources()
            if not s.startswith("flash_api")]
    def _resolve(s):
        if s.startswith("flex_flash_attention/"):
            return REPO_DIR / s
        return HOPPER_DIR / s
