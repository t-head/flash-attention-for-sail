...
TOL = {
    ...
    # TF32 tensor-core kernel (isolated fwd_f32/bwd_f32 set) vs the fp32
    # MATH fwd oracle / fp64 gradient oracle.
    FP32: dict(atol=1e-2, rtol=2e-2),
}
...
class TestCanUseAccept:
    ...
    # fp32 rides the ISOLATED fwd_f32/bwd_f32 TF32 kernel set; these mirror
    # the bf16/fp16 accept branches above for the float32 path (dropout is
    # kept at 0: the f32 set is validated for the dropout-free path only).
    def test_fp32_2d_mask(self):
        q, k, v = _mk(2, 4, 4, 256, 256, 64, dtype=FP32)
        mask = torch.tril(torch.ones(256, 256, device=DEVICE, dtype=torch.bool))
        mask[:64, 64:128] = True  # 2 intervals/row, still in the envelope
        _assert_close_arb(q, k, v, mask=mask)

    def test_fp32_no_mask(self):
        q, k, v = _mk(2, 4, 4, 256, 256, 64, dtype=FP32)
        _assert_close_arb(q, k, v, mask=None)

    def test_fp32_is_causal(self):
        q, k, v = _mk(2, 4, 4, 256, 256, 64, dtype=FP32)
        out = _run_arb(q, k, v, is_causal=True)
        ref = _ref_math(q, k, v, mask=_causal_mask(256, 256))
        torch.testing.assert_close(out, ref, **TOL[FP32])

    def test_fp32_4d_singleton_mask(self):
        q, k, v = _mk(2, 4, 4, 256, 256, 64, dtype=FP32)
        m2d = torch.tril(torch.ones(256, 256, device=DEVICE, dtype=torch.bool))
        _assert_close_arb(q, k, v, mask=m2d.unsqueeze(0).unsqueeze(0))

    def test_fp32_gqa_q8_kv2(self):
        q, k, v = _mk(2, 8, 2, 256, 256, 64, dtype=FP32)
        _assert_close_arb(q, k, v, mask=_causal_mask(256, 256))

    @pytest.mark.parametrize("hd", [64, 96, 128, 192, 256])
    def test_fp32_headdim_tiers(self, hd):
        q, k, v = _mk(1, 2, 2, 128, 128, hd, dtype=FP32)
        _assert_close_arb(q, k, v, mask=_causal_mask(128, 128))

    def test_fp32_rectangular_sq_neq_sk(self):
        # Non-square shapes pad q rows out to the bwd tile grid; the kernel
        # must stay inside the padded allocation (stale-memory hazard).
        q, k, v = _mk(2, 4, 4, 300, 512, 128, dtype=FP32)
        i = torch.arange(300, device=DEVICE).unsqueeze(1)
        j = torch.arange(512, device=DEVICE).unsqueeze(0)
        _assert_close_arb(q, k, v, mask=(i - j).abs() <= 32)

    def test_fp32_rectangular_gqa(self):
        q, k, v = _mk(2, 8, 2, 300, 512, 128, dtype=FP32)
        i = torch.arange(300, device=DEVICE).unsqueeze(1)
        j = torch.arange(512, device=DEVICE).unsqueeze(0)
        _assert_close_arb(q, k, v, mask=(i - j).abs() <= 32)
