...
# 配置路径和输出文件名
test_dir = "tests"  # 替换为你的测试文件路径
output_xml = "test-results.xml"
# 显式指定要运行的测试文件（确保所有文件以 .py 结尾）
test_files_cmds = [
    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_bwd_overflow"],
    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_bwd_transpose"],
    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_bwd_varlen_overflow"],
    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_causal"],
    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_deterministic"],
    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_kvcache"],
#    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_output"], expect 4 failures
    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_qkvpacked"],
    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_race_condition"],
#    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_splitkv"], # expect 8 failures
    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_varlen_causal"],
    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_varlen_deterministic"],
    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_varlen_output"],
    [os.path.join(test_dir, "test_flash_attn.py"), "test_flash_attn_varlen_qkvpacked"],
]
temp_xml_files = []
