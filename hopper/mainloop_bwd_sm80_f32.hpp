...
struct CollectiveMainloopBwdSm80 {
    ...
    // ISOLATED fp32 (TF32) kernel set gate (mirrors mainloop_fwd_sm80.hpp):
    // the AIU/TSM smem->reg swizzle cubes are 16-bit geometry, so float
    // rides the stock (non-AIU) path. Compile-time per instantiation; the
    // 16-bit build keeps identical values.
#if defined(USE_PPU) && USE_AIU
    ...
    static constexpr bool Use_CVT_SWZL_LD = Use_aiu && ArchTag::kMinComputeCapability >= 89 && (kHeadDim == 128 || kHeadDim == 256) && (kBlockN >= 128);
        ...
        std::conditional_t<
            ...
            std::conditional_t<
                ...
                std::conditional_t<
                    ...
                    // TiledMma Tiles below (mirrors mainloop fwd).
...
#endif
    // fp32 tf32 atom has K=8 instead of 16 (see MMA_Atom_Arch).
    static constexpr int kMmaTileK = sizeof(Element) == 4 ? 8 : 16;
    using TiledMmaSdP = TiledMMA<
        MMA_Atom_Arch,
        AtomLayoutSdP,
        Tile<Int<16 * CUTE_STATIC_V(size<0>(AtomLayoutSdP{}))>, Int<(MmaSdPEvenN ? 16 : 8) * CUTE_STATIC_V(size<1>(AtomLayoutSdP{}))>, Int<kMmaTileK>>>;
    ...
    using TiledMmadKV = TiledMMA<
        MMA_Atom_Arch,
        AtomLayoutdKV,
        Tile<Int<16 * CUTE_STATIC_V(size<0>(AtomLayoutdKV{}))>, Int<(MmadKVEvenN ? 16 : 8) * CUTE_STATIC_V(size<1>(AtomLayoutdKV{}))>, Int<kMmaTileK>>>;
    ...
    using TiledMmadQ = TiledMMA<
        MMA_Atom_Arch,
        AtomLayoutdQ,
        Tile<Int<16 * CUTE_STATIC_V(size<0>(AtomLayoutdQ{}))>, Int<(MmadQEvenN ? 16 : 8) * CUTE_STATIC_V(size<1>(AtomLayoutdQ{}))>, Int<kMmaTileK>>>;
    ...
    // changes the layout.
#if defined(USE_PPU) && USE_AIU
    // ISOLATED fp32 kernel set: the plain AIU atom is 16-bit geometry; float
    // falls back to the swizzled atom (self-consistent with kSwizzleBase=2).
    using SmemLayoutAtomQdO = std::conditional_t<Use_aiu,
...
#if defined(USE_PPU) && USE_AIU
    // ISOLATED fp32 kernel set: see SmemLayoutAtomQdO above.
    using SmemLayoutAtomKV = std::conditional_t<Use_aiu,
        Layout<Shape<_8, Int<kBlockKGmem>>, Stride<Int<kBlockKGmem>, _1>>,
        decltype(
            composition(Swizzle<kSwizzle, kSwizzleBase, kSwizzleBase>{},
    ...
    // ISOLATED fp32 kernel set: Swizzle<B, M, S> requires S >= B, and 4-byte
    // elements have kSwizzleBase = 2, so the PdS swizzle depth is capped at 2
    // (self-consistent layout; 16-bit builds keep the original value).
    static constexpr int kSwizzlePdS_eff = sizeof(Element) == 4 ? 2 : kSwizzlePdS;
    using SmemLayoutAtomPdS = decltype(
        composition(Swizzle<kSwizzlePdS_eff, kSwizzleBase, kSwizzleBase>{},
    ...
    // ISOLATED fp32 kernel set: LDSM atoms are b16-only; 4-byte elements
    // use plain elementized copies (one float per op), same as mainloop fwd.
    ...
    using SmemCopyAtomHalf = std::conditional_t<sizeof(Element) == 4,
        Copy_Atom<UniversalCopy<uint32_t>, Element>,
        Copy_Atom<SM75_U32x2_LDSM_N, Element>>;
    using SmemCopyAtomTransposedHalf = std::conditional_t<sizeof(Element) == 4,
        Copy_Atom<UniversalCopy<uint32_t>, Element>,
    ...
    // ISOLATED fp32 kernel set: every AIU/TSM smem->reg op below is 16-bit
    // geometry. They sit behind dependent templates so the float kernel set
    // never instantiates them (their Copy_Traits are invalid for 4-byte
    // elements); 16-bit picks the identical types as before.
    template <typename Elem, typename = void>
    struct SmemCopyOpQ_t { using type = UniversalCopy<uint32_t>; };
    template <typename Elem>
    struct SmemCopyOpQ_t<Elem, std::enable_if_t<(sizeof(Elem) == 2)>> {
        using type = std::conditional_t<
            Use_CVT_SWZL_LD,
            PPU0015_TSM_LD_SWZL_CVT_LOCAL<Elem, 8, 64, kBlockM, kHeadDimPadding, SdP_swapAB, false, kBlockM / 8 * kHeadDim / 64, true, 128, 64>,
            std::conditional_t<
                ArchTag::kMinComputeCapability >= 89,
                PPU0015_TSM_LD_SWZL<Elem, kBlockM, kBlockKGmem, SdP_swapAB, false, kHeadDim / kBlockKGmem>,
                PPU0010_TSM_LD_SWZL<Elem, kBlockM, kBlockKGmem, false, false, kHeadDim / kBlockKGmem>>>;
    };
    template <typename Elem, typename = void>
    struct SmemCopyOpQt_t { using type = UniversalCopy<uint32_t>; };
    template <typename Elem>
    struct SmemCopyOpQt_t<Elem, std::enable_if_t<(sizeof(Elem) == 2)>> {
        using type = std::conditional_t<
            Use_CVT_SWZL_LD,
            PPU0015_TSM_LD_SWZL_CVT_LOCAL<Elem, 8, 64, kBlockM, kHeadDimPadding, !dKV_swapAB, true, kBlockM / 8 * kHeadDim / 64, true, 128, 64>,
            std::conditional_t<
                ArchTag::kMinComputeCapability >= 89,
                PPU0015_TSM_LD_SWZL<Elem, kBlockM, kBlockKGmem, !dKV_swapAB, true, kHeadDim / kBlockKGmem>,
                PPU0010_TSM_LD_SWZL<Elem, kBlockM, kBlockKGmem, false, true, kHeadDim / kBlockKGmem>>>;
    };
    template <typename Elem, typename = void>
    struct SmemCopyOpK_t { using type = UniversalCopy<uint32_t>; };
    template <typename Elem>
    struct SmemCopyOpK_t<Elem, std::enable_if_t<(sizeof(Elem) == 2)>> {
        using type = std::conditional_t<
            Use_CVT_SWZL_LD,
            PPU0015_TSM_LD_SWZL_CVT_LOCAL<Elem, 8, 64, kBlockN, kHeadDimPadding, !SdP_swapAB, false, kBlockN / 8 * kHeadDim / 64, true, 128, 64>,
            std::conditional_t<
                ArchTag::kMinComputeCapability >= 89,
                PPU0015_TSM_LD_SWZL<Elem, kBlockN, kBlockKGmem, !SdP_swapAB, false, kHeadDim / kBlockKGmem>,
                PPU0010_TSM_LD_SWZL<Elem, kBlockN, kBlockKGmem, false, false, kHeadDim / kBlockKGmem>>>;
    };
    template <typename Elem, typename = void>
    struct SmemCopyOpKVt_t { using type = UniversalCopy<uint32_t>; };
    template <typename Elem>
    struct SmemCopyOpKVt_t<Elem, std::enable_if_t<(sizeof(Elem) == 2)>> {
        using type = std::conditional_t<
            Use_CVT_SWZL_LD,
            PPU0015_TSM_LD_SWZL_CVT_LOCAL<Elem, 8, 64, kBlockN, kHeadDimPadding, !dQ_swapAB, true,  kBlockN / 8 * kHeadDim / 64, true, 64, 1024, true>,
            std::conditional_t<
                ArchTag::kMinComputeCapability >= 89,
                PPU0015_TSM_LD_SWZL<Elem, kBlockN, kBlockKGmem, !dQ_swapAB, true, kHeadDim / kBlockKGmem>,
                PPU0010_TSM_LD_SWZL<Elem, kBlockN, kBlockKGmem, false, true, kHeadDim / kBlockKGmem>>>;
    };
    using SmemCopyAtomQ = Copy_Atom<typename SmemCopyOpQ_t<Element>::type, Element>;
    using SmemCopyAtomQt = Copy_Atom<typename SmemCopyOpQt_t<Element>::type, Element>;
    using SmemCopyAtomK = Copy_Atom<typename SmemCopyOpK_t<Element>::type, Element>;
    using SmemCopyAtomKVt = Copy_Atom<typename SmemCopyOpKVt_t<Element>::type, Element>;
    // PdS (P/dS) smem->reg atoms feeding the dKV / dQ mmas. Same deferred
    // treatment: float never instantiates the 16-bit TSM ops.
    ...
    template <typename Elem, typename = void>
    struct SmemCopyAtomPdSt_AIU_t { using type = Copy_Atom<UniversalCopy<uint32_t>, Elem>; };
    template <typename Elem>
    struct SmemCopyAtomPdSt_AIU_t<Elem, std::enable_if_t<(sizeof(Elem) == 2)>> {
#if PPU1v5_R2S_SLICE_LAYOUT
        using type = Copy_Atom<std::conditional_t<
            Use_CVT_SWZL_LD_PdS,
            PPU0015_TSM_LD_SWZL_CVT_LOCAL<Elem, 8, 64, kBlockM, kBlockN_PdS_safe, dKV_swapAB, true, kBlockM / 8 * kBlockN_PdS_safe / 64, true, 128, 64>,
            PPU0015_TSM_LD_SWZL<Elem, kBlockM, kBlockNGmem, dKV_swapAB, true, kBlockN / kBlockNGmem>>, Elem>;
#else
        // we purpose to pre-transpose P in share memory and load in non-transpose fashion
        using type = Copy_Atom<PPU0010_TSM_LD_SWZL<Elem, kBlockN, kBlockMGmem, false, false, kBlockM / kBlockMGmem>, Elem>;
#endif
    };
    template <typename Elem, typename = void>
    struct SmemCopyAtomdS_AIU_t { using type = Copy_Atom<UniversalCopy<uint32_t>, Elem>; };
    template <typename Elem>
    struct SmemCopyAtomdS_AIU_t<Elem, std::enable_if_t<(sizeof(Elem) == 2)>> {
#if PPU1v5_R2S_SLICE_LAYOUT
        using type = Copy_Atom<std::conditional_t<
            Use_CVT_SWZL_LD_PdS,
            PPU0015_TSM_LD_SWZL_CVT_LOCAL<Elem, 8, 64, kBlockM, kBlockN_PdS_safe, dQ_swapAB, false, kBlockM / 8 * kBlockN_PdS_safe / 64, true, 64, 128, true>,
            PPU0015_TSM_LD_SWZL<Elem, kBlockM, kBlockNGmem, dQ_swapAB, false, kBlockN / kBlockNGmem>>, Elem>;
#else
        using type = Copy_Atom<PPU0010_TSM_LD_SWZL<Elem, kBlockN, kBlockMGmem, false, true, kBlockM / kBlockMGmem>, Elem>;
#endif
    };
    using SmemCopyAtomPdSt_AIU = typename SmemCopyAtomPdSt_AIU_t<Element>::type;
    using SmemCopyAtomdS_AIU = typename SmemCopyAtomdS_AIU_t<Element>::type;
#else
    ...
    // ISOLATED fp32 kernel set: AIU_LOAD is 16-bit geometry; defer it behind
    // dependent templates so float never instantiates it.
    template <typename Elem, typename = void>
    struct Gmem_copy_struct_Q_t { using type = GmemCopyStruct; };
    template <typename Elem>
    struct Gmem_copy_struct_Q_t<Elem, std::enable_if_t<(sizeof(Elem) == 2)>> {
        using type = std::conditional_t<
            Use_CVT_SWZL_LD,
            PPU0015_AIU_LOAD<cute::C<bits_per_aiu_Q_Cvt>, Elem, false, 8, kBlockKGmem>,
            std::conditional_t<
                ArchTag::kMinComputeCapability >= 89,
                PPU0015_AIU_LOAD<cute::C<bits_per_aiu_Q>, Elem, false, kBlockM, kBlockKGmem>,
                PPU0010_AIU_LOAD<cute::C<bits_per_aiu_Q>, Elem, false>>>;
    };
    template <typename Elem, typename = void>
    struct Gmem_copy_struct_KV_t { using type = GmemCopyStruct; };
    template <typename Elem>
    struct Gmem_copy_struct_KV_t<Elem, std::enable_if_t<(sizeof(Elem) == 2)>> {
        using type = std::conditional_t<
            Use_CVT_SWZL_LD,
            PPU0015_AIU_LOAD<cute::C<bits_per_aiu_KV_Cvt>, Elem, false, 8, kBlockKGmem>,
            std::conditional_t<
                ArchTag::kMinComputeCapability >= 89,
                PPU0015_AIU_LOAD<cute::C<bits_per_aiu_KV>, Elem, false, kBlockN, kBlockKGmem>,
                PPU0010_AIU_LOAD<cute::C<bits_per_aiu_KV>, Elem, false>>>;
    };
    using Gmem_copy_struct_Q = typename Gmem_copy_struct_Q_t<Element>::type;
    using Gmem_copy_struct_KV = typename Gmem_copy_struct_KV_t<Element>::type;
    // CVT path loads 8 x kBlockKGmem gmem tiles, the other paths load a full kBlockM/kBlockN block.
    // ISOLATED fp32 kernel set: float reuses the stock CP_ASYNC tiled copy.
