...
void run_flash_fwd_combine(Flash_fwd_params &params, hggcStream_t stream, bool enable_pdl) {
    ...
    int max_seqlen_q = params.num_splits_dynamic_ptr ? (params.seqlen_q <= 16 ? params.seqlen_q : 16) : params.seqlen_q;
    int num_blocks_m = cute::ceil_div(max_seqlen_q * params.h, kBlockM);
#else
