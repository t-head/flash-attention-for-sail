...
// Single source of truth for the PPU CVT + swizzled-smem backward load path.
// Both mainloop_bwd_sm80.hpp and epilogue_bwd.hpp gate on this, and they MUST agree: with the
// mainloop on and the epilogue off (or vice versa) the dK/dV accumulators are permuted on one side
// only and the results are silently wrong, with nothing failing at compile time.  Defined here, in
// the header both of them already include, so flipping it is a one-line change; can also be
// overridden from the build with -DCVT_LAYOUT=0.
// Note this only ever affects configs that pass the Use_CVT_SWZL_LD gate
// (kHeadDim 128/256 with kBlockN >= 128 on compute capability >= 89); every other headdim compiles
...
template<bool A_in_regs=false, bool B_in_regs=false, bool SwapAB=false,
         ...
         typename TiledMma, typename TiledCopyA, typename TiledCopyB,
         typename ThrCopyA, typename ThrCopyB, typename Hook>
CUTLASS_DEVICE void gemm_sm80_QK(Tensor0 &acc, Tensor1 &tCrA, Tensor2 &tCrB, Tensor3 const& tCsA,
                              Tensor4 const& tCsB, TiledMma tiled_mma,
                              TiledCopyA smem_tiled_copy_A, TiledCopyB smem_tiled_copy_B,
                              ThrCopyA smem_thr_copy_A, ThrCopyB smem_thr_copy_B, Hook fn) {
    if constexpr (SwapAB) {
...
#define DEBUG_T_ID 0
        ...
        for (int i = 0; i < size<2>(tCrA); ++i) {
            if (!A_in_regs) { cute::copy(smem_tiled_copy_A, tCsA(_, _, i), tCrA_copy_view(_, _, i)); }
            if (!B_in_regs) { cute::copy(smem_tiled_copy_B, tCsB(_, _, i), tCrB_copy_view(_, _, i)); }
            if (cute::thread(DEBUG_T_ID)) {
                print(tCrA_copy_view);
                print("\n");
                print(tCrB_copy_view);
                print("\n");
                #pragma unroll
                for (int n = 0; n < size<1>(tCrA_copy_view); ++n) {  // outermost: M-direction tile (MMA_M)
                    #pragma unroll
                    for (int m = 0; m < size<0>(tCrA_copy_view); ++m) {
                        printf("tCrA_copy_view[%d][%d][%d] = %.3f\n", m, n, i, static_cast<float>(tCrA_copy_view(m, n, i)));
                    }
                }
                #pragma unroll
                for (int n = 0; n < size<1>(tCrB_copy_view); ++n) {  // outermost: mode1 tile (MMA_N for B)
                    #pragma unroll
                    for (int m = 0; m < size<0>(tCrB_copy_view); ++m) {
                        printf("tCrB_copy_view[%d][%d][%d] = %.3f\n", m, n, i, static_cast<float>(tCrB_copy_view(m, n, i)));
                    }
                }
            }
            if constexpr (!std::is_same_v<Hook, std::nullptr_t>) {
            ...
            if (cute::thread(DEBUG_T_ID)) {
                ...
                print("\n");
                #pragma unroll
                for (int n = 0; n < size<1>(acc); ++n) {  // outermost: M-direction tile (MMA_M)
                    #pragma unroll
                    for (int m = 0; m < size<0>(acc); ++m) {
                        #pragma unroll
                        for (int k = 0; k < size<2>(acc); ++k) {
                            printf("acc[%d][%d][%d] (i=%d) = %.3f\n", m, n, k, i, static_cast<float>(acc(m, n, k)));
                        }
                    }
                }
            }
