...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor> mha_bwd(
    ...
    // Non-PPU path: single dispatch per hdim, so min == max for semaphore sizing.
    int const kBlockM_min = kBlockM;
    int const kBlockN_min = kBlockN;
#endif
    auto round_multiple = [](int x, int m) { return (x + m - 1) / m * m; };
    ...
    // The AIU path uses Is_even_MN=true for gmem loads (no bounds checking).
    ...
    // Use kBlockM_min (smallest dispatch kBlockM) since m_block count = ceil(seqlen_q / kBlockM)
    // is largest with the smallest kBlockM, requiring the largest semaphore buffer.
    at::Tensor dq_semaphore = torch::empty({(seqlen_q + kBlockM_min - 1) / kBlockM_min, batch_size, num_heads}, opts.dtype(torch::kInt32));
    params.dq_semaphore = dq_semaphore.data_ptr<int>();
    if (num_heads_k != num_heads && params.deterministic) {
        // TODO: do we need to zero them out?
        // Use kBlockN_min (smallest dispatch kBlockN) since n_block count = ceil(seqlen_k / kBlockN)
        // is largest with the smallest kBlockN, requiring the largest semaphore buffer.
        at::Tensor dk_semaphore = torch::empty({(seqlen_k + kBlockN_min - 1) / kBlockN_min, batch_size, num_heads_k}, opts.dtype(torch::kInt32));
        at::Tensor dv_semaphore = torch::empty({(seqlen_k + kBlockN_min - 1) / kBlockN_min, batch_size, num_heads_k}, opts.dtype(torch::kInt32));
