...
@torch.library.custom_op("flash_attn_3_ops::_flash_attn_forward", mutates_args=(), device_types="cuda")
def _flash_attn_forward(
    ...
    softmax_lse_accum = rest[1] if len(rest) > 1 and rest[1] is not None else torch.tensor([], device=out.device)
    return out, softmax_lse, out_accum, softmax_lse_accum


@torch.library.register_fake("flash_attn_3_ops::_flash_attn_forward")
...
@torch.library.custom_op("flash_attn_3_ops::_flash_attn_backward", mutates_args=("dq", "dk", "dv"), device_types="cuda")
...
@torch.library.register_fake("flash_attn_3_ops::_flash_attn_backward")
def _flash_attn_backward_fake(
    ...
    # PPU kBlockM: only depends on head_size_rounded
    kBlockM = 128 if head_size_rounded <= 64 else 64
...
def setup_context(ctx, inputs, output):
    # inputs matches _flash_attn_forward's signature order
    # Tensor args: q=0, k=1, v=2, k_new=3, v_new=4, qv=5, out=6,
    #   cu_seqlens_q=7, cu_seqlens_k=8, cu_seqlens_k_new=9,
    #   seqused_q=10, seqused_k=11
    # Scalar/int args: max_seqlen_q=12, max_seqlen_k=13
    # softmax_scale=23, causal=24, window_size_left=25, window_size_right=26
    # attention_chunk=27, softcap=28, sm_margin=33
    q, k, v = inputs[0], inputs[1], inputs[2]
    cu_seqlens_q = inputs[7]
    cu_seqlens_k = inputs[8]
    seqused_q = inputs[10]
    seqused_k = inputs[11]
    ...
    ctx.max_seqlen_q = inputs[12]
    ctx.max_seqlen_k = inputs[13]
