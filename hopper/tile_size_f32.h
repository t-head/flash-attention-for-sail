...
#ifdef USE_PPU
// Return {kBlockM, kBlockN, kNWarps, kStages, Q_in_regs}
constexpr std::tuple<int, int, int, int, bool> tile_size_fwd_ppu_f32(
    ...
    if (element_size == 2) {
        ...
        if (headdim <= 128) {
            // P4 sweep (causal s1024/s4096/GQA-4:1, 2026-08): Stages=1 beat
            // Stages=2 by ~18% — with 4-byte elements the double-buffered
            // smem (2x Q/K/V tiles) leaves the pipeline starved vs a leaner
            // single-stage footprint; kBlockN=64/128 tie, keep 64.
            return {64, 64, 4, 1, true};
        } else if (headdim <= 192) {
            return {64, 32, 4, 2, true};
        } else {
            // P7 sweep (d256 causal b{1,4}xs{1024,4096}, 2026-08): Stages=1
            // edges out Stages=2 (-0.5%); kBlockN=64 (2.4x worse, smem
            // pressure with 4-byte buffers) and kBlockM=64 (static_assert
            // reject at 8 warps / 2.2x worse at 4 warps) are both dead ends.
            // Keep 8 warps for the faster d256 transpose.
            return {128, 32, 8, 1, true};
        }
    }
}
#endif