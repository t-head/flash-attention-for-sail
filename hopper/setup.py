...
FA3_HLLM_BUILD = 'FA3_HLLM_BUILD' in os.environ.keys() and os.environ['FA3_HLLM_BUILD'] == "1"
FA3_HOLMES_WHEEL = 'FA3_HOLMES_WHEEL' in os.environ.keys() and os.environ['FA3_HOLMES_WHEEL'] == "1"
# FA3_HLLM_USE_ADDR stands for use addr-format block_table
FA3_HLLM_USE_ADDR = 'FA3_HLLM_USE_ADDR' in os.environ.keys() and os.environ['FA3_HLLM_USE_ADDR'] == "1"
if FA3_HLLM_USE_ADDR:
    assert FA3_HLLM_BUILD, "when FA3_HLLM_USE_ADDR is set, FA3_HLLM_BUILD must also be set."
...
if FA3_HOLMES_WHEEL:
    PACKAGE_NAME = "flash_attn_3_holmes"
...
def _write_ninja_file(path,
    ...
    if USE_PPU:
        flags = [s.replace("code=compute_", "code=sm_") for s in flags]   # remove ptx code in elf
        if DISABLE_SM80:
            flags = [s.replace("sm_80", "sm_89") for s in flags]
        if DISABLE_SM89:
            flags = [s.replace("sm_80", "sm_80a") for s in flags]
...
def is_offline_build() -> bool:
    ...
    """
    return True if USE_PPU else check_env_flag("FLASH_ATTENTION_OFFLINE_BUILD", "")
...
if not SKIP_CUDA_BUILD:
    ...
    cutlass_dir = repo_dir / "csrc" / ("cutlass3" if USE_PPU else "cutlass")
    ...
    feature_args = (
        ...
        + (["-DFLASHATTENTION_DISABLE_SM86"] if USE_PPU else [])
    ...
    # Diff head dimensions (kHeadDim != kHeadDimV) for SM80 forward kernels.
    # When SOFTCAP is enabled, the _softcapall_ batch files already include these variants.
    # When SOFTCAP is disabled, we must include the individual files explicitly.
    # 64_256 and 64_512 only exist for bf16/fp16 (not e4m3); 192_128 exists for all dtypes.
    HEAD_DIMENSIONS_FWD_SM80_DIFF = (
        (["64_256", "64_512"] if not DISABLE_HDIM64 else [])
        + (["192_128"] if not DISABLE_HDIM192 else [])
    ) if DISABLE_SOFTCAP else []
    HEAD_DIMENSIONS_FWD_SM80_ALL = HEAD_DIMENSIONS_FWD_SM80 + HEAD_DIMENSIONS_FWD_SM80_DIFF
    ...
    # For SM8x (PPU), PackGQA is always true in the dispatch code (Arch < 90).
    # The non-split non-packgqa files instantiate PackGQA=false, so the _packgqa
    # files (with PackGQA=true) are always needed even when PACKGQA is disabled.
    PACKGQA = [""] + (["_packgqa"] if (not DISABLE_PACKGQA or USE_PPU) else [])
    ...
    sources_fwd_sm80 = ([f"instantiations/flash_fwd_hdim{hdim}_{dtype}{paged}{split}{softcap}_sm80.cu"
                         for hdim, dtype, split, paged, softcap in itertools.product(HEAD_DIMENSIONS_FWD_SM80_ALL, DTYPE_FWD_SM80, SPLIT, PAGEDKV, SOFTCAP_ALL)
                         if not (dtype == "e4m3" and hdim in ["64_256", "64_512"])]
                        if not USE_PPU else
                        [f"instantiations/flash_fwd_hdim{hdim}_{dtype}{paged}{split}{softcap}{packgqa}_sm80.cu"
                         for hdim, dtype, split, paged, softcap, packgqa in itertools.product(HEAD_DIMENSIONS_FWD_SM80_ALL, DTYPE_FWD_SM80, SPLIT, PAGEDKV, SOFTCAP_ALL, PACKGQA)
                         if not ((packgqa and split) or (dtype == "e4m3" and hdim == 96) or (dtype == "e4m3" and hdim in ["64_256", "64_512"]))])
    ...
    if USE_PPU:
        nvcc_flags.extend(
            [
                ...
                "-mllvm",
                "-ppu-volatile-yield=false",
                "-mllvm",
                "-sort-copy-before-coalesce=true",
            ]
        )
        feature_args.extend(
            [
        ...
        nvcc_flags.extend(
            [
                "-Xfatbin",
                "--compress-all",
                # "-save-temps",   # uncomment it for compilation debug
            ]
        )
