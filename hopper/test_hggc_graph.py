"""CUDA graph regression tests for FA3 on FlashAttention 2.7.4.
...
def poison_paged_tail(cache, page_table_cpu, live_seqlen, table_capacity_seqlen):
    """Fill the graph-visible but logically unused V region with NaNs."""
    page_size = cache.shape[1]
    first_page = live_seqlen // page_size
    last_page = (table_capacity_seqlen + page_size - 1) // page_size
    for batch_idx in range(page_table_cpu.shape[0]):
        for logical_page in range(first_page, last_page):
            ...
            end = table_capacity_seqlen % page_size if logical_page == last_page - 1 else page_size
            if end == 0:
                end = page_size
...
def test_varlen_graph_replay_shorter_seqused_q(
    ...
    def run_fa():
        # In 2.7.4, seqused_q/k precede max_seqlen_q/k and the function
        # always returns (out, softmax_lse).
        out, _ = flash_attn_varlen_func(
            ...
            seqused_q=seqused_q,
            seqused_k=None,
            max_seqlen_q=capacity_seqlen_q,
...
@pytest.mark.parametrize("capture_seqlen_k", [512])
@pytest.mark.parametrize("table_capacity_seqlen_k", [2048])
@torch.inference_mode()
def test_paged_kvcache_graph_replay_shorter_cache_seqlen(
        table_capacity_seqlen_k, capture_seqlen_k,
        replay_seqlen_k, page_size, d, causal, dtype
):
    """Replay runtime KV lengths below the fixed page-table capacity.

    The 2.7.4 kvcache API derives its graph bound from page-table capacity;
    it has no independent max_seqlen_k argument. Capture with a live prefix,
    then poison every unused V row up to table capacity on each replay.
    """
    if DISABLE_PAGEDKV:
        pytest.skip("paged KV disabled in this build")
    device = "cuda"
    assert replay_seqlen_k < capture_seqlen_k < table_capacity_seqlen_k
    ...
    cache_seqlens = torch.full(
        (batch_size,), capture_seqlen_k, device=device, dtype=torch.int32
    )
    ...
    for live_seqlen_k in (capture_seqlen_k, replay_seqlen_k, capture_seqlen_k):
        v_cache.copy_(v_cache_saved)
        poison_paged_tail(
            v_cache, page_table_cpu, live_seqlen_k, table_capacity_seqlen_k
        )
