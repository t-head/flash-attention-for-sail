                ...
                BOOL_SWITCH(return_softmax, ReturnSoftmaxConst, [&] {
                    ALIBI_SWITCH(params.alibi_slopes_ptr != nullptr, Has_alibi, [&] {
                        SOFTCAP_SWITCH(params.softcap > 0.0, Is_softcap, [&] {
                            ...
                            // auto kernel = &flash_fwd_kernel<Kernel_traits, false, false, false, false, false, false, false>;
...
#ifdef USE_PPU
    const bool is_even_MN = params.cu_seqlens_q == nullptr && params.cu_seqlens_k == nullptr && params.seqlen_k % 128 == 0 && params.seqlen_q % kBlockM == 0;
    HEADDIM_EQAL(Headdim, params.d_value, [&] {
        BOOL_SWITCH(is_even_MN, IsEvenMNConst, [&] {
            // constexpr static int kBlockN = Headdim <= 64 ? 256 : (Headdim <= 128 ? 128 : 64);
            constexpr static int kBlockN = (Headdim >=128 || IsEvenMNConst) ? 64 : Headdim >= 128 ? 128 : 256;
            if (params.seqlenq_ngroups_swapped)
                run_flash_splitkv_fwd<Flash_fwd_kernel_traits<Headdim, 16, 64, 1, false, false, T, kHeadDimV>, Is_causal>(params, stream);
                // run_flash_splitkv_fwd<Flash_fwd_kernel_traits<Headdim, 16, 16, 1, false, false, T>, Is_causal>(params, stream);
            else
                run_flash_splitkv_fwd<Flash_fwd_kernel_traits<Headdim, 64, kBlockN, 4, false, false, T, kHeadDimV>, Is_causal>(params, stream);
        });
#else
        run_flash_splitkv_fwd<Flash_fwd_kernel_traits<Headdim, 64, kBlockN, 4, false, false, T>, Is_causal>(params, stream);
#endif
    });
}
...
void run_mha_fwd_hdim64(Flash_fwd_params &params, hggcStream_t stream) {
    ...
    int _num_sm = get_num_sm(get_current_device()); (void)_num_sm;
    DROPOUT_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
        if constexpr(!Is_dropout) {
            // Using 8 warps is 18% slower for seqlen=2k, 2 warps is 5% slower
            // Using block size (64 x 256) is 27% slower for seqlen=2k
            // Using block size (256 x 64) is 85% slower for seqlen=2k, because of register spilling
            int blocks = params.b * params.h * ((params.seqlen_q + 128 - 1) / 128);
            if (float(blocks / _num_sm) < 4.0f) {
    ...
    DROPOUT_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
        if constexpr(!Is_dropout) {
            ...
            if (is_sm8x) {
                ...
                int blocks = params.b * params.h * ((params.seqlen_q + 128 - 1) / 128);
                if (float(blocks / _num_sm) < 3.0f) {
                    run_flash_fwd<Flash_fwd_kernel_traits<Headdim, 64, 64, 4, true, false, T>, Is_dropout, Is_causal>(params, stream);
                } else {
                    run_flash_fwd<Flash_fwd_kernel_traits<Headdim, 128, 64, 4, false, false, T>, Is_dropout, Is_causal>(params, stream);
                }
            }
...
template<typename T, bool Is_causal>
void run_mha_fwd_hdim160(Flash_fwd_params &params, hggcStream_t stream) {
...
void run_mha_fwd_hdim256(Flash_fwd_params &params, hggcStream_t stream) {
    ...
    hggcError_t status_ = hggcDeviceGetAttribute(
    ...
    DROPOUT_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
        ...
        //FIXME:: ppu smem is 164 per CU, always need to use 128 x 64
