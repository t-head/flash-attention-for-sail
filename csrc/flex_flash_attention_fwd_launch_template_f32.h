...
#include "../hopper/tile_size_f32.h"
#include "../hopper/tile_scheduler.hpp"
#include "../hopper/mainloop_fwd_sm80_f32.hpp"   // TYPE PROVIDER ONLY
#include "../hopper/epilogue_fwd_f32.hpp"
#include "flex_flash_attention_fwd_kernel_sm80_f32.h"
...
void run_flex_flash_attention_fwd(FlexFlashAttentionFwdParams &params, hggcStream_t stream) {
    ...
    static constexpr std::tuple<int, int, int, int, bool> kBlockMN_kNWarps_Stages_RS = tile_size_fwd_ppu_f32(
...
void run_mha_flex_flash_fwd_dispatch_impl(FlexFlashAttentionFwdParams &params, hggcStream_t stream) {
...
#else
    {
        ...
        // sizeof(T) != 4: the ISOLATED fp32 kernel set only ships kBlockM=64
        // tiles (stage-2 double buffering with 4-byte elems would overflow smem).
        bool const use_kblockm_128 = !force_kbm64 && hint_allows_kbm128 &&
            sizeof(T) != 4 && kHeadDim == 128 && params.seqlen_q > 64 &&
