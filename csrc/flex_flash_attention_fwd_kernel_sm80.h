...
#include "../hopper/mainloop_fwd_sm80.hpp"   // TYPE PROVIDER ONLY — we don't call mma()
...
public:
        ...
        SeqlenInfo_t const& seqlen_info)
    {
        ...
        Tensor gQ = local_tile(make_mix_tensor_like(mQ),
        ...
        Tensor gK = local_tile(make_mix_tensor_like(mK),
        ...
        Tensor mV = make_tensor(
            ...
            _, _, bidh_kv, !is_varlen_k ? bidb_kv : 0);
        Tensor gV = local_tile(make_mix_tensor_like(mV),
                               select<2, 1>(TileShape_MNK_PV{}),
                               make_coord(_, _0{}));  // (N, K, _)
            ...
            Tensor tOrP = [&]() -> auto {
                if constexpr (ArchTag::kMinComputeCapability >= 89) {
                    if constexpr (std::is_same_v<Element, float>) {
                        ...
                        return flash::convert_acc_to_tf32_Aregs(tSrS);
                    } else {
