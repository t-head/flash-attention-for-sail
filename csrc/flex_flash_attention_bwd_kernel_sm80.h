...
#include "../hopper/mainloop_bwd_sm80.hpp"   // TYPE PROVIDER ONLY — we don't call mma()
...
public:
        ...
        int n_block, int bidh, int bidb)
    {
        ...
        Tensor gQ     = local_tile(make_mix_tensor_like(mQ),  select<0, 2>(TileShape_MNK{}), make_coord(_, _0{}));  // (M, K, _)
        Tensor gdO    = local_tile(make_mix_tensor_like(mdO), select<0, 2>(TileShape_MNK{}), make_coord(_, _0{}));  // (M, K, _)
        Tensor gK     = local_tile(make_mix_tensor_like(mK),  select<1, 2>(TileShape_MNK{}), make_coord(n_block, _0{}));  // (N, K)
        Tensor gV     = local_tile(make_mix_tensor_like(mV),  select<1, 2>(TileShape_MNK{}), make_coord(n_block, _0{}));  // (N, K)
        ...
        Tensor tdSsdS = r2s_thr_copy_PdS.partition_D(cute::conditional_return<!SdP_swapAB>(sdS_slice, sdSt_slice));
        Tensor tPsP   = r2s_thr_copy_PdS.partition_D(cute::conditional_return<!SdP_swapAB>(sP_slice, sPt_slice));
#else
        ...
        // ── Load K/V once per work tile (they are fixed for this n_block) ──
        {
            ...
            if constexpr (CollectiveMainloop::Use_CVT_SWZL_LD) {
                ...
                flash::copy</*Is_even_MN=*/true>(gmem_tiled_copy_V, tVgV, tVsV, t0KVcKV, tVpV);
            }
#else
            #pragma unroll
            ...
            if constexpr (CollectiveMainloop::Use_CVT_SWZL_LD) {
                ...
                flash::copy</*Is_even_MN=*/true>(gmem_tiled_copy_K, tKgK, tKsK, t0KVcKV, tKpK);
            }
#else
            #pragma unroll
        ...
        auto load_Q_LSE = [&] (int const m_block, int const smem_pipe_write) {
            ...
            if constexpr (CollectiveMainloop::Use_CVT_SWZL_LD) {
                ...
                flash::copy</*Is_even_MN=*/true>(gmem_tiled_copy_Q, tQgQ_cur, tQsQ_cur, t0QcQ, tQpQ);
            }
#else
            #pragma unroll
        ...
        auto load_dO_dPsum = [&] (int const m_block, int const smem_pipe_write) {
            ...
            if constexpr (CollectiveMainloop::Use_CVT_SWZL_LD) {
                ...
                flash::copy</*Is_even_MN=*/true>(gmem_tiled_copy_dO, tdOgdO_cur, tdOsdO_cur, t0QcQ, tQpQ);
            }
#else
            #pragma unroll
        ...
        int m_block, int bidh, int bidb)
    {
        ...
        auto load_KV = [&] (int const n_block_cur) {
            ...
            if constexpr (CollectiveMainloop::Use_CVT_SWZL_LD) {
                ...
                flash::copy</*Is_even_MN=*/true>(gmem_tiled_copy_V, tVgV(_, _, _, n_block_cur), tVsV, t0KVcKV, tVpV);
            }
#else
            #pragma unroll
            for (int m = 0; m < size<1>(tVsV); ++m) {
            ...
            if constexpr (CollectiveMainloop::Use_CVT_SWZL_LD) {
                ...
                flash::copy</*Is_even_MN=*/true>(gmem_tiled_copy_K, tKgK(_, _, _, n_block_cur), tKsK, t0KVcKV, tKpK);
            }
#else
            #pragma unroll
            for (int m = 0; m < size<1>(tKsK); ++m) {
